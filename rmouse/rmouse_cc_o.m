...
%           % preallocate current channel combos
%           cccPosPeakDecay{ccix1,ccix2}=ninanny;
%           cccNegPeakDecay{ccix1,ccix2}=ninanny;
%           for cpi=1:r(i).ni
%             tmpPeaks{j,cpi}=cat(2,tmpr.posPeakT{cpi}+tOffs,tmpr.posPeak{cpi});
...
%             pix{j}=tmpix(1:min(5,length(tmpix)));
        ...
        if nPaths==1
          ...
          if nPaths<2
            ...
            hold on
            plot((discrete2cont(ccw(1)+1:ccw(2)+1,WP.osi*.001)+WP.osi*.001*.5)',...
      ...
      for j=1:nPairs
        ...
        if ki==1        
          ...
          % update expected lag:
          % expLag(ccix1,ccix2)=cccPeakTMn{ccix1,ccix2};
        end
      end
    end % for: # diagonals
