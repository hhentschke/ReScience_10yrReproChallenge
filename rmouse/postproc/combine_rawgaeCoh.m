function R=combine_rawgaeCoh(RInfo)
% ** function R=combine_rawgaeCoh(RInfo)
% A variant of combine_r (see below). Deals only with rawgaeCoh results and
...
rfn='rcomb_wtAtropine_rawgaeCoh';
  ...
  for ci=1:n2
    for ri=1:n1
      ...
      if ~isempty(DS.rawCh)
        ...
        if ri==1
          % template (single data set): permute rawgaeCoh variables such
          % that we end up with a 3D array with nAllLFPCh rows, 1 column,
          % nAllLFPCh slices
