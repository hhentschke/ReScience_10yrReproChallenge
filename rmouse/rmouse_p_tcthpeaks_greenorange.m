function rmouse_p_tcthpeaks_greenorange(p_etsl,boe,eoe)
...
[intrvls,nix,intrvlLen]=mkintrvls([boe eoe]*.001,'resol',WP.osi*1e-6,'ilen',tps,'olap',0,'border','include');
...
% replace amplitudes by marker sizes: largest amplitude of 99.5 % events=16
pp_apm=ceil(16*pp_apm./repmat(pp_co,size(pp_apm,1),1));
np_apm=ceil(16*np_apm./repmat(np_co,size(np_apm,1),1));
% no blobs from outer space, please: restrict size to 20 pts
pp_apm(pp_apm>20)=20;
np_apm(np_apm>20)=20;
...
for ii=1:nIntrvls
  ...
  for chInd=AP.LFPInd
    ...
    for i=tmpi'
      for gg=pp_apm(i,chInd):-3:1
        ph=plot(pp_tpm(i,chInd),AP.LFPccInd(chInd),'o');
        set(ph,'markersize',gg,'color',[gg 20 gg]/20);
      end
    end
    % --- neg peaks:
    % find all points in current interval
    ...
    % plot each point individually to be able to set its marker size
    for i=tmpi'
      for gg=np_apm(i,chInd):-3:1      
        ph=plot(np_tpm(i,chInd),AP.LFPccInd(chInd),'o');
        set(ph,'markersize',gg,'color',([25.5 17.0 0]+gg)/45.5);
      end
    end
  end
  % print?
