function [c,lags,scale] = xcorr(x,varargin)
...
%     'coeff_ub' - both 'unbiased' and 'coeff' scaling is done
...
%   $Revision: 1.14 $  $Date: 2001/04/02 20:20:42 $ 
%   modified (scaling) hh feb 2002
%
%   References:
...
% Scale as specified
[c,msg,scale] = scaleXcorr(c,xIsMatrix,scaleType,autoFlag,M,maxlag,lags,x,varargin{:});
error(msg);
...
%----------------------------------------------------------------
function [c,msg,scale] = scaleXcorr(c,xIsMatrix,scaleType,autoFlag,...
...
case {'coeff','coeff_ub'}
  ...
  % hh
  if strcmp(scaleType,'coeff_ub'),
    % Scales the correlation ADDITIONALLY (well, multiplicatively, mathematically speaking) by M/(M-abs(lags)).
    ub_scale = ((M-abs(lags)).')./M;
    ub_scale(ub_scale<=0)=1; % avoid divide by zero, when correlation is zero
    
    if xIsMatrix,
      ub_scale = repmat(ub_scale,1,size(c,2));
    end
    
    c = c./ub_scale;
    scale=scale.*ub_scale;
  end  
end	
...
if isempty(scaleType),
  ...
  scaleOpts = {'biased','unbiased','coeff','coeff_ub','none'};
  indx = strmatch(lower(scaleType),scaleOpts,'exact');
