% This routine generates plots of theta, beta, gamma and raw data triggered to
...
excIntv=[-175 175];
excIntvPts=cont2discrete(excIntv,WP.osi*.001,'intv',1);
eIx=excIntvPts(1):excIntvPts(2);
% borders to cut off from excerpts (filter) in ms
border=50;
killPts=cont2discrete(border,WP.osi*.001);
...
for i=explV
  if ~isempty(r(i).iPts)
    ...
    for chaI=1:2
      ...
      % convert to pts
      thPeakT=cont2discrete(thPeakT,WP.osi*.001);
      ...
      if exist([DS.dpath '\' DS.abfFn '.mat'],'file')
        rawD=matDload([DS.dpath '\' DS.abfFn '.mat'],'start',0,'stop',discrete2cont(r(i).iPts(segIx(end),2),WP.osi*1e-6,'intv',1),...
          'channels',{rawCh(chInd).nm},'verbose',verbose);
      else
        rawD=abfload([DS.dpath '\' DS.abfFn '.abf'],'start',0,'stop',discrete2cont(r(i).iPts(segIx(end),2),WP.osi*1e-6,'intv',1),...
          'channels',{rawCh(chInd).nm},'verbose',verbose);
      end
...
%       offs=cumsum([0 co1-co2(2)  co2(1)-co3]);
      ...
      ph=plot(mean(excD-fastD,2)+offs(1),'w');      
      set(ph,'linewidth',1);
