% This routine generates plots of theta, beta and raw data triggered to
% individual peaks of theta. The idea is to find out to what degree beta is
% the first harmonic of theta. It needs the result from spectral analysis.
% *** must be called from within rmouse as a special job ***
...
orient portrait
labelscale('fontSz',10,'scaleFac',1.0,'lineW',.75,'markSz',4);
...
for i=explV
  if ~isempty(r(i).iPts)
    ...
    for chaI=1:2
      ...
      spci=spci+1;
      % identify segments with beta power above threshold
      [tmpCo]=cumh((r(i).rawBePE{chIdx,chIdx})',.01,'p',betaPowCo);
      segIx=find(r(i).rawBePE{chIdx,chIdx}>=tmpCo);
      nSeg=length(segIx);
      
      % identify theta peaks above threshold
      [tmpCo]=cumh(-1*negPA{chIdx},.01,'p',thetaPeakCo);

      % time stamp list of potentially interesting peaks in pts
      thPeakIx=(-1*negPA{chIdx}>=tmpCo);
      thPeakT=negPT{chIdx}(thPeakIx);
      % purge double peaks
      ...
      % load data: beginning to last point of last identified segment
      ...
      beD=bafi(excD,si,AP.betaCFreq,'rs',AP.rs);
      % cut
      excD(killPts,:)=[];
      beD(killPts,:)=[];
      gaD(killPts,:)=[];
      ...
      subplot(1,nChannel,spci), hold on
      ph=plot(excD(:,plotIx)+offs(1));
      set(ph,'color',[.6 .6 .6]);
      
      ph=plot(beD(:,plotIx)+offs(2));
      set(ph,'color',[.6 .6 .6]);
      ...
      set(ph,'linewidth',2);
      ph=plot(mean(beD,2)+offs(2),'k');      
      set(ph,'linewidth',2);
      ph=plot(mean(gaD,2)+offs(3),'k');      
      set(ph,'linewidth',2);
      
      ph=plot(mean(excD-beD,2)+offs(1),'r');      
      set(ph,'linewidth',1);

      ph=plot(mean(excD-gaD,2)+offs(1),'g');      
      set(ph,'linewidth',1);
...
if ~isempty(AP.printas{1}),
  for i=1:length(AP.printas)
    ...
    print(pa,[figName '_thetaTrigRaw' ext]);
  end
end
