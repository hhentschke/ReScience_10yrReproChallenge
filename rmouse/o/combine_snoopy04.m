...
% the specific job here: extract peaks from AVERAGED thgaeCC
        ...
        if ~isempty(DS.rawCh)
          ...
          for bi=1:length(bix)
            for rvi=1:length(rv)
              ...
              switch rv{rvi}
                case {'thgaeCCMn','thgaeCCPeakMn','thgaeCCPeakTMn','detheCCPeakMn','detheCCPeakTMn','thgaeCCPosPeakDecayMn','thgaeCCZScore','thgaeCCZTestP'}
                  if ~isempty(tmpr)
                    y=tempo;          
                    ...
                    elseif strcmpi(rv{rvi},'thgaeCCMn')
                      tmpP=evdeal(tmpr,'idx',{'closepeak'},'tP',AP.ccLagPts+1);
                      tmpr=discrete2cont(tmpP.tlNegPeakT-AP.ccLagPts-1,si*.001,'intv',0);
                    end
                    y(AP.LFPccInd)=tmpr;
                  end
                otherwise
